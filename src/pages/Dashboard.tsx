...
const Dashboard: React.FC = () => {
  ...
  const stats = [
    ...
    { label: 'Active Courses', value: courses.length.toString(), trend: 4.2, icon: GraduationCap, color: 'bg-emerald-500/10 text-emerald-600' },
    { label: 'My Enrollments', value: enrolledCourses.length.toString(), trend: 0, icon: BookOpen, color: 'bg-amber-500/10 text-amber-600' },
        ...
        <div className="md:flex grid grid-cols-s gap-3 items-end justify-between">
          <div>
            ...
            <p className="text-on-surface-variant mt-2">
              Welcome back, John. Here's what's happening across your
              organization today.
            </p>
          </div>
          ...
          {stats.map((stat, index) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div
                  className={`p-3 rounded-2xl ${stat.color} group-hover:scale-110 transition-transform`}
                >
                  <stat.icon size={24} />
                </div>
                <div
                  className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-lg ${stat.trend > 0 ? "bg-emerald-500/10 text-emerald-600" : "bg-red-500/10 text-red-600"}`}
                >
                  {stat.trend > 0 ? (
              ...
              <h3 className="text-2xl font-display font-bold text-on-surface mt-1">
                {stat.value}
              </h3>
            </motion.div>
          ))}
              ...
              {loading ? (
                ...
                enrolledCourses.slice(0, 3).map((enrollment) => (
                  <div
                    ...
                    className="flex items-center gap-6 p-4 hover:bg-surface-container-low rounded-2xl transition-all group cursor-pointer border border-transparent hover:border-outline-variant"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="px-2 py-0.5 bg-primary-container text-white text-[10px] font-bold rounded uppercase tracking-wider">
                      ...
                      <h4 className="font-display font-bold text-on-surface truncate group-hover:text-primary transition-colors">
                ...
                <div className="text-center py-12 bg-surface-container-low rounded-2xl border border-dashed border-outline-variant">
                  <GraduationCap
                    size={48}
                    className="mx-auto text-on-surface-variant opacity-20 mb-4"
                  />
              ...
              {ACTIVITIES.map((activity, index) => (
                <div key={activity.id} className="flex gap-4 relative">
                  ...
                  <img
                    src={activity.avatar}
                    alt={activity.user}
                    className="w-12 h-12 rounded-xl object-cover shrink-0 border-2 border-surface-container-lowest"
                    referrerPolicy="no-referrer"
                  />
                  <div>
